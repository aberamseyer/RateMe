            ...
            mFirebaseDatabase.child("users").child(mFireBaseUserId).addListenerForSingleValueEvent(new ValueEventListener() { // reads the data once
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    ...
                    if(dataSnapshot.exists()) { // the very existence of this snapshot means the user has registered, we're good to go. mark them as active
                        ...
                        mFirebaseDatabase.child("users").child(mFireBaseUserId).child("active").setValue(true);
                        mFirebaseDatabase.child("users").child(mFireBaseUserId).child("rating").setValue(0);
                        mFirebaseDatabase.child("users").child(mFireBaseUserId).child("numberOfRatings").setValue(0);
                        mFirebaseDatabase.child("users").child(mFireBaseUserId).child("ratings").setValue(new ArrayList<Double>(0));
                        startActivity(new Intent(MainActivity.this, RegistrationActivity.class).putExtra("method", "new"));
                    }
                }
